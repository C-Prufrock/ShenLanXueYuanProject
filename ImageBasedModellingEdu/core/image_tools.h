...
template <typename T>
typename Image<T>::Ptr  //建立一个函数模板；功能在于对图像进行上采样；
rescale_double_size_supersample (typename Image<T>::ConstPtr img)
{
    int const iw = img->width();  //获取图像宽度；
    int const ih = img->height(); //获取图像高度；
    int const ic = img->channels(); //获取图像的通道数；
    int const ow = iw << 1;  //对图像宽度×2；
    int const oh = ih << 1;  //图像高度×2；

    typename Image<T>::Ptr out(Image<T>::create()); //创建图像；
    out->allocate(ow, oh, ic); //分配上采样图像的宽度,高度,通道数；

    int witer = 0;     //作为图像遍历的指针，将图像视为一列元素；
    for (int y = 0; y < oh; ++y)   // 循环赋值；
    {
        bool nexty = (y + 1 < oh);  //如果当前数小于现图像数目2倍则nexty为1；
        int yoff[2] = { iw * (y >> 1), iw * ((y + nexty) >> 1) };  // 建立数组，yoff[0]为当前数目/2,yoff[1]为当前图像高度+1 后/2；
        for (int x = 0; x < ow; ++x)  //遍历图像列；
        {
            bool nextx = (x + 1 < ow);  //若当前x+1小于当前图像宽度×2
            int xoff[2] = { x >> 1, (x + nextx) >> 1 };  //xoff【0】为当前x/2,xoff【2】为图像列宽度+1后/2
            T const* val[4] =
            {
            ...
            for (int c = 0; c < ic; ++c, ++witer)  //遍历通道；
                out->at(witer) = math::interpolate  //对图像进行插值处理；重要的是使用了interpolate函数；每个值诚意0.25相加获得当前值；
...
blur_gaussian (typename Image<T>::ConstPtr in, float sigma)
{
    if (in == nullptr)   //如果输入图像为空，则报错；
        throw std::invalid_argument("Null image given");

    /* Small sigmas result in literally no change. */
    if (MATH_EPSILON_EQ(sigma, 0.0f, 0.1f))  //如果模糊系数在-0.1-0.1之间，则对起进行上采样
        return in->duplicate();

    int const w = in->width();   //获取图像宽度；
    int const h = in->height();   //获取图像高度；
    int const c = in->channels();  //获取图像通道数
    int const ks = std::ceil(sigma * 2.884f); // Cap kernel at 1/128
    std::vector<float> kernel(ks + 1);

    /* Fill kernel values. */
    for (int i = 0; i < ks + 1; ++i)
        kernel[i] = math::gaussian((float)i, sigma);   //填充高斯模板中 各自的数值；
