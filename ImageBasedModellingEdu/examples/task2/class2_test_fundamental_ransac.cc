...
int  calc_ransac_iterations (double p,
    ...
    /* 计算迭代次数*/
    double ChanceOfOneOutlier=1-pow(p,K);
    num_iterations=log(1-z)/log(ChanceOfOneOutlier);
    return static_cast<int>(math::round(num_iterations)); //round为四舍五入为整数;
}

/**
 * \description 给定基础矩阵和一对匹配点，计算匹配点的sampson 距离，用于判断匹配点是否是内点,
 * 计算公式如下：
 *              SD = (x'Fx)^2 / ( (Fx)_1^2 + (Fx)_2^2 + (x'F)_1^2 + (x'F)_2^2 )   SD=||Ch(x)||/(JTJ)-1;
 * @param F-- 基础矩阵
 * @param m-- 匹配对
 * @return
 */
...
int main(int argc, char *argv[]){
    ...
    std::ifstream in("/home/true/深蓝学院/ImageBasedModellingEdu/examples/task2/correspondences.txt");
    //assert(in.is_open());
   ...
   /* double Change=pow(inlier_ratio,n_samples);
    std::cout<<log(1-0.99)/log(1-Change)<<std::endl;*/  //测试代码;
    ...
    for(int i=0; i<n_iterations; i++){
        ...
        /*2.0 8点法估计相机基础矩阵*/
        FundamentalMatrix F;
        calc_fundamental_8_point(pset1, pset2,F); //计算F矩阵;

        /*3.0 统计所有的内点个数*/
        std::vector<int> inlier_indices = find_inliers(corr_all, F, inlier_thresh);  //获得内点的indice;
...
//                      << "%)" << std::endl;
    ...
    for(int i=0; i< best_inliers.size(); i++){
        corr_f.push_back(corr_all[best_inliers[i]]);  //获取标号代表的数据,用于最小二乘法解算F矩阵;
    }
