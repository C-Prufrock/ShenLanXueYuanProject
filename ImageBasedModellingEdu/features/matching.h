...
public:
    ...
    {
        /**
         * The length of the descriptor. Typically 128 for SIFT, 64 for SURF.描述子的长度 SIFT为128位；4*4*8
         */
        int descriptor_length;

        /**
         * Requires that the ratio between the best and second best matching 需要第一最优匹配与第二最优匹配之间的比率；
         * distance is below some threshold. If this ratio is near 1, the match 二者之间距离将低于一定的阈值，如果比率为1的话，匹配是模糊不清的，
         * is ambiguous. Good values are 0.8 for SIFT and 0.7 for SURF.  对于SIFT来说 好的比率是0.8,SURF来说 比率是0.7；
         * Set to 1.0 to disable the test.
         */
        float lowe_ratio_threshold;     //第一最优匹配与第二最优匹配之间的比率；

        /**
         * Does not accept matches with distances larger than this value.如果匹配距离大于该值,则不认为二者为匹配；
    ...
    struct Result
    {
        ...
        std::vector<int> matches_1_2;  //从set1 向set2进行匹配；
        /* Matches from set 2 in set 1. */
        std::vector<int> matches_2_1; //从set2 向set1进行匹配；
    };
...
template <typename T>
void
Matching::oneway_match (Options const& options,  //单向匹配；
    T const* set_1, int set_1_size,
    T const* set_2, int set_2_size,
    std::vector<int>* result)
{
    ...
    // 与最近邻距离的阈值
    float const square_dist_thres = MATH_POW2(options.distance_threshold); //返回最大距离的平方；

    // 以描述子为特征，计算每个特征点的最近邻和次近邻
    NearestNeighbor<T> nn;
    nn.set_elements(set_2);//返回set_2;
    // 特征点的个数
    nn.set_num_elements(set_2_size);//返回特征点的个数；
    // 设置特征描述子的维度 sift 128, surf 64
    nn.set_element_dimensions(options.descriptor_length); //返回描述子的长度；

    for (int i = 0; i < set_1_size; ++i) //集合1
    {
        // 每个特征点最近邻搜索的结果
        ...
        // 标准1： 与最近邻的距离必须小于特定阈值
        if (nn_result.dist_1st_best > square_dist_thres)  //如果第一最优匹配大于阈值；
            continue;
