...
public:
    ...
    struct Result
    {
        T dist_1st_best;  //最近邻 第一最优距离；
        T dist_2nd_best;  //第二最优距离；
        int index_1st_best;  //第一最优距离的index；
        int index_2nd_best;  //第二最优距离的index；
    };
...
public:
    ...
    /** For SfM, this is the descriptor length. *///描述子的长度；
    void set_element_dimensions (int element_dimensions);
    /** For SfM, this is the number of descriptors. *///描述子数量
    void set_num_elements (int num_elements);
    /** Find the nearest neighbor of 'query'. *///寻找最近邻；
...
private:
    int dimensions;   //维度；
    int num_elements; //元素数目；
    T const* elements;  //元素地址；
};
...
{
    this->elements = elements;   //返回元素的elements；
}
